...
PRIVATE void	tty_dev_write	(TTY* tty, int color);
	...
	while (tty->ibuf_cnt) {
		...
		if (tty->tty_left_cnt) {
			if (ch >= ' ' && ch <= '~') { /* printable */
				...
				assert(color == 0);
				outCharWithColor(tty->console, ch, color);
...
PRIVATE void tty_do_write(TTY* tty, MESSAGE* msg)
{
	...
	while (i) {
		...
		for (j = 0; j < bytes; j++)
			outCharWithColor(tty->console, buf[j], msg->COLOR);
		i -= bytes;
		p += bytes;
	}
